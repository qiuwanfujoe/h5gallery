...
BOOL unZipFile(NSString *zipFilePath, NSString *unzipFolderPath, BOOL isDeleteZipFile) {
  if (access([zipFilePath UTF8String], 0) != 0 || unzipFolderPath.length == 0) {
    return NO;
  }
 
  ZipArchive *zip = [[ZipArchive alloc] init];
  BOOL openRet = [zip UnzipOpenFile:zipFilePath];
  BOOL unzipRet = NO;
  BOOL closeRet = NO;
  if (openRet) {
    unzipRet = [zip UnzipFileTo:unzipFolderPath overWrite:YES];
    if (unzipRet) {
      closeRet = [zip UnzipCloseFile];
    }
  }
    BOOL deleZipRet = YES;
  
    if (isDeleteZipFile) {
        deleZipRet = (remove([zipFilePath UTF8String])==0);
    }
  return openRet && unzipRet && closeRet && deleZipRet;
}
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
//			TLog(@"%ld entries in the zip file",globalInfo.number_entry);
		}
	}
